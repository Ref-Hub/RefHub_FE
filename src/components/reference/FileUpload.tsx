...
import { Link, Image as ImageIcon, FileText, File, X, Plus, GripVertical } from 'lucide-react';
...
export default function FileUpload({
  ...
  const validateFile = (file: File, type: 'image' | 'pdf' | 'file'): boolean => {
  ...
  const renderUploadField = (file: FileItem, index: number) => {
    if (file.type === 'link') {
      return (
        <input
          ...
          className="flex-1 px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-[#62BA9B]"
        />
      );
    }
        ...
        return (
          <div className="space-y-2">
            <div className="flex flex-wrap gap-2">
              ...
              {images.length < 5 && (
                <label className="w-20 h-20 flex items-center justify-center border-2 border-dashed border-gray-300 rounded-lg hover:border-[#62BA9B] transition-colors cursor-pointer">
                  <input
                    type="file"
                    accept="image/*"
                    multiple
    ...
    return (
      <label className="flex-1 flex items-center justify-center h-32 border-2 border-dashed border-gray-300 rounded-lg hover:border-[#62BA9B] transition-colors cursor-pointer">
        <input
          type="file"
        ...
        {files.map((file, index) => (
          ...
          >
            ...
            <div className="relative w-32">
              <select
                value={file.type}
                onChange={(e) => handleTypeChange(index, e.target.value as FileItem['type'])}
